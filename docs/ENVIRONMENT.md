...
```text
PORT=4000
DATABASE_URL=postgresql://autopartiq:autopartiq@localhost:5432/autopartiq?schema=public
JWT_SECRET=replace-with-a-long-random-value
FRONTEND_URL=http://localhost:5173
ANTHROPIC_API_KEY=optional-for-live-ai
```

Start the local PostgreSQL database with:
...
For production, use a hosted PostgreSQL database and update `DATABASE_URL` accordingly.
