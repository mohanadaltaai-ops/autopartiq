...
export default function Customer({ tab }) {
  ...
  return <div className="p-4 space-y-4">
    ...
    {homeTab === 'new' ? <RequestForm token={token} onDone={async () => { await load(); setToast({ message: 'Your request was submitted successfully.', type: 'success' }); setHomeTab('requests'); }} /> : <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <h2 className="font-black text-slate-900">My Requests</h2>
        <select className="p-2 rounded-xl border bg-white text-xs font-bold" value={requestFilter} onChange={e => setRequestFilter(e.target.value)}>
          <option value="PENDING">Pending</option>
          <option value="CANCELLED">Cancelled</option>
          <option value="ALL">Pending + Cancelled</option>
        </select>
      </div>
...
function RequestCard({ req, token, reload, onToast }) {
  ...
  async function cancel() {
    try {
      ...
      onToast?.({ message: 'Request cancelled successfully.', type: 'success' });
      await reload();
    } catch (e) {
      setError(e.message);
  ...
  return <div className="bg-white rounded-2xl border p-4 space-y-3 shadow-sm">
    <button onClick={() => setOpen(value => !value)} className="w-full text-left flex justify-between gap-3">
      ...
      <div className="text-right">
        <span className={`text-[10px] px-2 py-1 rounded-full h-fit ${req.status === 'CANCELLED' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{req.status}</span>
