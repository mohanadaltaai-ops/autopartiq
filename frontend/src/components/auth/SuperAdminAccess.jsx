...
export default function SuperAdminAccess({ onBack }) {
  ...
  async function submit() {
    ...
    setError('');
    try {
      const result = await api('/auth/super-admin/login', {
        method: 'POST',
        body: { identifier, password: secret }
      });
  ...
  return (
    <>
      ...
      <h1 className="text-xl font-black text-slate-900">{t('adminAccess')}</h1>
      ...
      <button
        onClick={submit}
        disabled={!identifier || !secret || loading}
        className="bg-purple-700 text-white rounded-2xl py-4 font-bold disabled:opacity-40"
      >
