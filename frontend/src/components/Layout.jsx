...
export default function Layout({ tab, setTab, children }) {
  ...
  return <div className="min-h-screen h-screen flex items-center justify-center p-4 bg-slate-600 overflow-hidden">
    <div className="phone-frame rounded-[42px] border-[7px] border-slate-950 shadow-2xl overflow-hidden flex flex-col relative apiq-page bg-[#F5F7FC] dark:bg-slate-950">
      <header className="shrink-0 bg-white/95 dark:bg-slate-950/95 backdrop-blur border-b border-slate-200 dark:border-slate-800 px-4 py-2 grid grid-cols-[82px_1fr_82px] items-center gap-2">
        <div className="relative z-10 flex items-center justify-start">
          <button onClick={toggleLanguage} className="w-10 h-10 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-100 text-xs font-black shadow-sm" title={t('language')}>
            {language === 'en' ? 'AR' : 'EN'}
          </button>
        </div>

        <button onClick={() => setTab(user?.adminPermission === 'ORDERS_ONLY' ? 'orders' : 'home')} className="min-w-0 flex flex-col items-center justify-center">
          <img src={logo} alt={getMarketAppName(language)} className="pointer-events-none h-12 w-[210px] object-contain scale-[1.65]" />
          <div className={`mt-0.5 text-[10px] font-black ${roleColor}`}>{roleLabel}</div>
        </button>

        <div className="relative z-10 flex items-center justify-end gap-2">

          <button onClick={() => setTab('profile')} className={`w-10 h-10 rounded-2xl border flex items-center justify-center shadow-sm transition ${tab === 'profile' ? 'bg-[#0F172A] text-white border-[#0F172A]' : 'bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-100 border-slate-200 dark:border-slate-800'}`} title={t('profile')}>
            <User size={18}/>
            <span className="sr-only">{t('profile')}</span>
          </button>

          {['CUSTOMER', 'SUPPLIER', 'ADMIN', 'SUPER_ADMIN'].includes(user?.role) && (
            <button onClick={toggleNotifications} className="relative w-10 h-10 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-100 flex items-center justify-center shadow-sm transition">
              <Bell size={17}/>
      ...
      {showNotifications && (
        <div className="absolute top-[76px] right-4 left-4 z-20 bg-white dark:bg-slate-950 rounded-[30px] border border-slate-200 dark:border-slate-800 shadow-2xl p-4 space-y-3 max-h-[68vh] overflow-y-auto">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="font-black text-slate-950 dark:text-white text-xl leading-tight">{t('notifications')}</h3>
              <div className="text-xs text-slate-500 dark:text-slate-400 font-semibold mt-1">{unreadCount} {t('unread') || 'unread'}</div>
            </div>
            <button onClick={() => setShowNotifications(false)} className="w-10 h-10 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-200 text-sm font-black">
              ×
            </button>
          </div>

          {notifications.length === 0 && (
            <div className="bg-slate-50 dark:bg-slate-900 border border-dashed border-slate-200 dark:border-slate-800 rounded-[24px] p-6 text-center text-sm font-bold text-slate-400">
              {t('noNotifications')}
            </div>
          )}
            ...
            {notifications.map(item => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left bg-white dark:bg-slate-900 rounded-[24px] p-3 text-slate-700 dark:text-slate-200 border border-slate-100 dark:border-slate-800 hover:bg-blue-50 dark:hover:bg-slate-800 hover:border-blue-100 dark:hover:border-slate-700 transition shadow-sm"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-2">
                        <div className="font-black text-slate-950 dark:text-white leading-tight">{notificationText(item, t)}</div>
      ...
      <nav className="shrink-0 bg-white/95 dark:bg-slate-950/95 backdrop-blur border-t border-slate-200 dark:border-slate-800 px-2 pt-2 pb-3 flex gap-1 overflow-x-auto">
        {tabs.map(([id, Icon, label]) => (
          <button key={id} onClick={() => setTab(id)} className={`min-w-[64px] flex-1 py-2.5 px-2 rounded-2xl text-[10px] font-black flex flex-col items-center gap-1 transition ${tab === id ? 'bg-[#0F172A] dark:bg-blue-700 text-white shadow-sm' : 'text-slate-400 dark:text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-900'}`}>
            <Icon size={18}/>
