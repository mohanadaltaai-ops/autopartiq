...
export default function SuperAdminEnroll({ token }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setMessage('Super Admin enrolled successfully.');
  ...
  return (
    <div className="bg-white rounded-2xl border p-4 shadow-sm space-y-3">
      <h2 className="font-black text-slate-900">Enroll Super Admin</h2>
      ...
      <input className="w-full p-3 rounded-xl border" type="password" placeholder="Password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} />
      ...
      <button onClick={submit} disabled={saving || !form.name || !form.phone || !form.username || !form.password} className="w-full py-3 rounded-2xl bg-purple-700 text-white font-black disabled:opacity-40">
        {saving ? 'Saving...' : 'Enroll Super Admin'}
      </button>
    </div>
  );
}
